/**
 * QuanChain Wallet - BIP-39 Mnemonic Implementation
 * Mnemonic generation and seed derivation
 */

// BIP-39 English wordlist (2048 words)
// This is a subset for the implementation - in production use the full list
const WORDLIST = [
...
/**
 * Generate a mnemonic phrase asynchronously.
 * Kept for completeness, but the wallet uses the sync version below.
 * @param {number} strength - Entropy bits (128=12 words, 256=24 words)
 * @returns {Promise<string>} - Space-separated mnemonic words
 */
export async function generateMnemonicAsync(strength = 256) {
...
async function entropyToMnemonic(entropy) {
  ...
  const checksumBits = entropy.length / 4; // 1 bit per 32 bits of entropy
  ...
  // Add checksum bits
  bits += hashArray[0].toString(2).padStart(8, '0').slice(0, checksumBits);
...
function entropyToMnemonicSync(entropy) {
  // Simplified checksum using first byte of entropy XOR
  let checksumByte = 0;
  for (const byte of entropy) {
    checksumByte ^= byte;
  }
...
// Override to use sync version
export function generateMnemonic(strength = 256) {
  const entropyBytes = strength / 8;
  const entropy = crypto.getRandomValues(new Uint8Array(entropyBytes));
  return entropyToMnemonicSync(entropy);
}
